...
void setup() {
    ...
    Serial.println(F("T,GT,PRES,TIN,TEXT,LAT,LNG,ALT,SPD,CRS,CNT,V,AGE"));
#else
    mprls.begin();
    //dht.setup(PIN_DHT);
    ...
    if (active_file.open(LOG_FILENAME, O_RDWR | O_CREAT | O_AT_END)) {
        //active_file.println(F("t,gps_t,pressure,temp_int,temp_ext,lat,long,altitude,speed,course,sat_count,mV,age"));
        active_file.close();
    } else {
#if DEBUG
...
void loop() {
    ...
    noTone(PIN_BUZZER);
    if (is_buzz_time()) //Serial.println("debug"); //tone(PIN_BUZZER, 400);
    ...
    if (is_buzz_time()) //Serial.println("debug"); //tone(PIN_BUZZER, 1500);
